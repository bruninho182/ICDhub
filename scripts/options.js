document.addEventListener("DOMContentLoaded", () => {
  ...
  chrome.storage.local.get(
    ["configMaster", "posicaoBarra", "emailAssunto", "emailCorpo"],
    (res) => {
      if (res.configMaster) {
      ...
      if (res.emailAssunto)
        document.getElementById("emailAssuntoPadrao").value = res.emailAssunto;
      if (res.emailCorpo)
        document.getElementById("emailCorpoPadrao").value = res.emailCorpo;
    },
  );
  ...
  btnSalvarPref.onclick = () => {
    ...
    chrome.storage.local.set({ posicaoBarra: posicao }, () => {
      alert(
        "✅ Preferência de posição salva! Atualize o WhatsApp para aplicar a mudança.",
      );
    });
  };
  ...
  document.getElementById("btnExportar").onclick = () => {
    chrome.storage.local.get(null, (todosOsDados) => {
      ...
      const blob = new Blob([JSON.stringify(todosOsDados, null, 2)], {
  ...
  document.getElementById("btnGerarPDF").onclick = function () {
    if (!window.jspdf)
      return alert("❌ Erro: Biblioteca jspdf não encontrada!");
    ...
    doc.setFont("helvetica", "normal");
    const dCompra = formatarDataBR(
      document.getElementById("reciboDataCompra").value,
    );
    const dVisita = formatarDataBR(
      document.getElementById("reciboDataVisita").value,
    );
  ...
  function adicionarLinhaPreco(tipo = "", valor = "") {
    ...
    novaLinha.querySelector(".btn-remover-preco").onclick = () =>
      novaLinha.remove();
  }
